  ...
  $q = "SELECT id,name,pos,cat FROM prefix_downcats WHERE cat = ".$id." ORDER BY pos";
	...
	if ( db_num_rows($erg) > 0 ) {
 	  while ($row = db_fetch_object($erg) ) {
	    echo '<tr><td>'.$stufe.'- <a href="admin.php?archiv-downloads-S'.$row->id.'">'.$row->name.'</a></td>';
      echo '<td class="text-right"><a href="admin.php?archiv-downloads-E'.$row->id.'#edit" rel="tooltip" title="&auml;ndern"><i style="color:#2D9600;" class="fa fa-pencil-square-o"></i></a>
      <a href="javascript:Kdel('.$row->id.')" rel="tooltip" title="l&ouml;schen"><i style="color:#AD0000;" class="fa fa-trash-o"></i></a>
      <a href="admin.php?archiv-downloads-S'.$row->id.'-O'.$row->id.'-'.$row->pos.'-'.$row->cat.'" rel="tooltip" title="nach oben verschieben"><i style="color:#C64F00;" class="fa fa-long-arrow-up"></i></a>
      <a href="admin.php?archiv-downloads-S'.$row->id.'-U'.$row->id.'-'.$row->pos.'-'.$row->cat.'" rel="tooltip" title="nach unten verschieben"><i style="color:#C64F00;" class="fa fa-long-arrow-down"></i></a></td></tr>';
  ...
  case 'downloads' :
    ...
    if ( is_dir ( $f ) ) {
      ...
      foreach($ar_files as $v) {
        ...
        echo '<tr>';
        echo '<td>'.$v.'</td>';
        echo '<td><a style="margin-right:4px;" href="javascript:deleteFile(\''.$f.'\',\''.$v.'\')" rel="tooltip" title="l&ouml;schen"><i style="color:#AD0000;" class="fa fa-trash-o"></i></a>
<a style="margin-right:4px;" href="javascript:moveFile(\''.$f.'\',\''.$v.'\')" rel="tooltip" title="verschieben"><i class="fa fa-arrows"></i></a>
<a href="javascript:renFile(\''.$f.'\',\''.$v.'\')" rel="tooltip" title="umbenennen"><i style="color:#2D9600;" class="fa fa-pencil-square-o"></i></a>';
        echo '<td>'.get_upload_linked ( $f.'/'.$v ).'<a href="javascript:waehleThisFile(\''.$f.'/'.$v.'\')">w&auml;hlen</a></td>';
        echo '</tr>';
      }
    } else {
      echo '<tr><td colspan="3" class="Cmite">Verzeichnis nicht gefunden... <a href="?archiv-downloads-upload">&Uuml;bersicht</a></td></tr>';
    }
  ...
  # upload move file
    ...
    # downs
      ...
      $azk = $_ilch['cat'];
      $_ilch['datum'] = '<input type="checkbox" name="refdate">  <small>Datum aktualisieren</small>';
		} else {
