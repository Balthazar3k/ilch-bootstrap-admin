		...
		if ( $menu->get(2) == 'upload' ) {
				...
				if ( $tmp[1] == 'gif' OR $tmp[1] == 'png' OR $tmp[1] == 'jpg' OR $tmp[1] == 'jpeg')  {
					...
					if ( move_uploaded_file ( $_FILES['f']['tmp_name'], 'include/images/wars/'.$nname) ) {
					  @chmod('include/images/wars/'.$nname, 0777);
						...
						$msg = '<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Datei ('.$_FILES['f']['name'].' ) erfolgreich hochgeladen</div>';
					} else {
						$msg = '<div class="alert alert-danger" role="alert">Datei ( '.$_FILES['f']['name'].' ) nicht erfolgreich hochgeladen</div>';
					}
				} else {
			...
			if ( isset($_GET['d']) ) {
				if ( @unlink ( 'include/images/wars/'.$_GET['d'] ) ) {
					$msg = '<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Datei erfolgreich gel&ouml;scht</div>';
				} else {
			...
			if ( !is_writeable ( 'include/images/wars' ) ) {
				$msg = '<div class="alert alert-danger" role="alert">Bitte erst dem Ordner "images/wars" Schreibrechte (chmod 777) geben.</div>';
			}
			$mid = $_REQUEST['mid'];
			$wid = $_REQUEST['wid'];
			...
			foreach($ar as $v) {
				if ( file_exists ( 'include/images/wars/'.$wid.'_'.$mid.'.'.$v ) ) {
					...
					$file .= '&nbsp;&nbsp;<a href="javascript:deleteMap(\''.$v.'\')" rel="tooltip" title="l&ouml;schen"><i style="color:#AD0000;"class="fa fa-times"></i></a>';
					break;
				}
			}
		$tpl->set('wid' ,$wid);
      ...
      for($i=1;$i<=5;$i++) {
        ...
        foreach ($imgar as $v) {
          if ( file_exists('include/images/wars/'.$wid.'_'.$i.'.'.$v) ) {
			...
			$msg = '<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Erfolgreich gel&ouml;scht</div>';
		}
		if ( !empty($_POST['sub']) ) {
        ...
        if (isset($_POST['kalender']) AND $_POST['kalender'] == 'yes') {
          ...
          db_query("INSERT INTO prefix_kalender (time, title, text, recht) VALUES (".$timestamp.",'Lastwar gegen ".$_POST['gegner']."', '".$_POST['mtyp']." ".$_POST['mod']." in ".$_POST['game']." gegen [url=".$_POST['page']."]".$_POST['gegner']."[/url]\n\n[url=http://".$page."?wars-more-".$wid."]details des Wars[/url]', 0)");
				...
				$msg = '<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Erfolgreich eingetragen</div>';
			} else {
        ...
        if (isset($_POST['kalender']) AND $_POST['kalender'] == 'yes') {
          ...
          if (1 == db_result(db_query("SELECT COUNT(*) FROM prefix_kalender WHERE text like '%more-".$wid."]%'"),0)) {
            ...
            db_query("INSERT INTO prefix_kalender (time, title, text, recht) VALUES (".$timestamp.",'Lastwar gegen ".$_POST['gegner']."', '".$_POST['mtyp']." ".$_POST['mod']." in ".$_POST['game']." gegen [url=".$_POST['page']."]".$_POST['gegner']."[/url]\n\n[url=http://".$page."?wars-more-".$wid."]details des Wars[/url]', 0)");
				...
				$msg = '<div class="alert alert-success" role="alert"><i class="fa fa-check-square-o"></i> Erfolgreich ver&auml;ndert</div>';
			}
		}
		if ( !empty ($_GET['pkey']) ) {
