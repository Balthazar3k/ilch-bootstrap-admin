...
if (isset($_POST['ksub']) AND !empty($_POST['kontodaten'])) {
  ...
  if (!is_numeric($betrag)) {
    ...
    if (db_query("UPDATE `prefix_kasse` SET name = '$name', datum = '$datum', betrag = '$betrag', verwendung = '$verwendung' WHERE id = ".$menu->get(1)))
      echo '<div class="alert alert-warning" role="alert">Buchung wurde ge&auml;ndert ...</div> ';
    else echo '<div class="alert alert-danger" role="alert">Es ist ein Fehler aufgetreten, Buchung nicht ge&auml;ndert</div>';
    $menu->set_url(1,'');   
  } else {
    db_query("INSERT INTO prefix_kasse (datum,name,verwendung,betrag) VALUES ('".$datum."','".$name."','".$verwendung."',".$betrag.")");
    echo '<div class="alert alert-success" role="alert">Buchung wurde gespeichert ...</div> ';
  }
}
