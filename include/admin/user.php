...
function getfl($gid) {
    ...
    while ($row = db_fetch_assoc($erg)) {
        ...
        $liste .= $row['name'] . '<small>(' . $v . $r . $s . $m . ')</small>&nbsp;';
    }
    return($liste);
}
...
switch ($um) {
    ...
    case 'createNewUser' :
        ...
        if (!empty($_POST['name']) AND !empty($_POST['pass']) AND !empty($_POST['email']) and chk_antispam('adminuser_create', true)) {
            ...
            if (db_num_rows($erg) > 0) {
                $msg = '<div class="alert alert-danger text-center" role="alert">Der Name ist leider schon vorhanden!</div>';
            } else {
                $new_pass = $_POST['pass'];
		    ...
		    VALUES('" . $_POST['name'] . "','" . $passwordHash . "'," . $_POST['recht'] . ",'" . time() . "','" . time() . "','" . $_POST['email'] . "')");
                ...
                $msg = '<div class="alert alert-success" role="alert"><strong><i class="fa fa-check-square-o"></i></strong> Benutzer erfolgreich angelegt. <a href="admin.php?user" class="btn btn-default">zur&uuml;ck zur User Verwaltung</a></div>';
            }
        }
        $pass = '';
        $email = '';
